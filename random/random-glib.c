...
static void
rng_free (XiceRNG *rng)
{
  g_slice_free (XiceRNG, rng);
}

XiceRNG *
xice_rng_glib_new (void)
{
  XiceRNG *ret;

  ret = g_slice_new0 (XiceRNG);
  ret->seed = rng_seed;
...
XiceRNG *
xice_rng_glib_new_predictable (void)
{
  XiceRNG *rng;

  rng = xice_rng_glib_new ();
  rng->seed (rng, 0);
  return rng;
}
