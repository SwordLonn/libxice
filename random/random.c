...
static XiceRNG * (*xice_rng_new_func) (void) = NULL;
...
xice_rng_new (void)
{
  if (xice_rng_new_func == NULL)
    return xice_rng_glib_new ();
  else
    return xice_rng_new_func ();
}
...
xice_rng_generate_bytes_print (XiceRNG *rng, guint len, gchar *buf)
{
  ...
  for (i = 0; i < len; i++)
    buf[i] = chars[xice_rng_generate_int (rng, 0, strlen (chars))];
}
