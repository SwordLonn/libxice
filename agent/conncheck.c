...
static void priv_update_check_list_failed_components(XiceAgent *agent, Stream *stream);
static void priv_update_check_list_state_for_ready(XiceAgent *agent, Stream *stream, Component *component);
static guint priv_prune_pending_checks(Stream *stream, guint component_id);
static gboolean priv_schedule_triggered_check(XiceAgent *agent, Stream *stream, Component *component, XiceSocket *local_socket, XiceCandidate *remote_cand, gboolean use_candidate);
static void priv_mark_pair_nominated(XiceAgent *agent, Stream *stream, Component *component, XiceCandidate *remotecand);
static size_t priv_create_username(XiceAgent *agent, Stream *stream,
	guint component_id, XiceCandidate *remote, XiceCandidate *local,
	uint8_t *dest, guint dest_len, gboolean inbound);
static size_t priv_get_password(XiceAgent *agent, Stream *stream,
	XiceCandidate *remote, uint8_t **password);
 ...
 */
static gboolean priv_conn_check_initiate(XiceAgent *agent, CandidateCheckPair *pair)
{
	...
	pair->state = XICE_CHECK_IN_PROGRESS;
	xice_debug("Agent %p : pair %p state IN_PROGRESS", agent, pair);
 ...
 */
static gboolean priv_conn_check_unfreeze_next(XiceAgent *agent)
{
	...
	if (pair) {
		xice_debug("Agent %p : Pair %p with s/c-id %u/%u (%s) unfrozen.", agent, pair, pair->stream_id, pair->component_id, pair->foundation);
		pair->state = XICE_CHECK_WAITING;
		xice_debug("Agent %p : pair %p state WAITING", agent, pair);
		return TRUE;
	}
 ...
 * @return TRUE on success, and FALSE if no frozen candidates were found.
 */
static void priv_conn_check_unfreeze_related(XiceAgent *agent, Stream *stream, CandidateCheckPair *ok_check)
{
	GSList *i, *j;
	guint unfrozen = 0;

	g_assert(ok_check);
	g_assert(ok_check->state == XICE_CHECK_SUCCEEDED);
	g_assert(stream);
	...
	for (i = stream->conncheck_list; i; i = i->next) {
		...
		if (p->stream_id == ok_check->stream_id) {
			if (p->state == XICE_CHECK_FROZEN &&
				...
				xice_debug("Agent %p : Unfreezing check %p (after successful check %p).", agent, p, ok_check);
				p->state = XICE_CHECK_WAITING;
				xice_debug("Agent %p : pair %p state WAITING", agent, p);
				++unfrozen;
			}
		}
	}
		...
		for (i = agent->streams; i; i = i->next) {
			...
			for (j = stream->conncheck_list; j; j = j->next) {
				...
				if (p->stream_id == s->id &&
					...
					if (p->state == XICE_CHECK_FROZEN &&
						strcmp(p->foundation, ok_check->foundation) == 0) {
						xice_debug("Agent %p : Unfreezing check %p from stream %u (after successful check %p).", agent, p, s->id, ok_check);
						p->state = XICE_CHECK_WAITING;
						xice_debug("Agent %p : pair %p state WAITING", agent, p);
						++unfrozen;
 ...
 */
static gboolean priv_conn_check_tick_stream(Stream *stream, XiceAgent *agent, GTimeVal *now)
{
	...
	for (i = stream->conncheck_list; i; i = i->next) {
		...
		if (p->state == XICE_CHECK_IN_PROGRESS) {
			if (p->stun_message.buffer == NULL) {
				xice_debug("Agent %p : STUN connectivity check was cancelled, marking as done.", agent);
				p->state = XICE_CHECK_FAILED;
				xice_debug("Agent %p : pair %p state FAILED", agent, p);
			}
				...
				case STUN_USAGE_TIMER_RETURN_TIMEOUT:
				{
					...
					xice_debug("Agent %p : Retransmissions failed, giving up on connectivity check %p", agent, p);
					p->state = XICE_CHECK_FAILED;
					xice_debug("Agent %p : pair %p state FAILED", agent, p);
				...
				case STUN_USAGE_TIMER_RETURN_RETRANSMIT:
				{
					...
					xice_debug("Agent %p :STUN transaction retransmitted (timeout %dms).",
						agent, timeout);

					xice_socket_send(p->local->sockptr, &p->remote->addr,
		...
		if (p->state == XICE_CHECK_FROZEN)
			++frozen;
		else if (p->state == XICE_CHECK_IN_PROGRESS)
			++s_inprogress;
		else if (p->state == XICE_CHECK_WAITING)
			++waiting;
		else if (p->state == XICE_CHECK_SUCCEEDED)
			++s_succeeded;
		else if (p->state == XICE_CHECK_DISCOVERED)
			++s_discovered;

		if ((p->state == XICE_CHECK_SUCCEEDED || p->state == XICE_CHECK_DISCOVERED)
			&& p->nominated)
			++s_nominated;
		else if ((p->state == XICE_CHECK_SUCCEEDED ||
			p->state == XICE_CHECK_DISCOVERED) && !p->nominated)
		...
		if (agent->controlling_mode) {
			...
			for (n = 0; n < stream->n_components; n++) {
				for (k = stream->conncheck_list; k; k = k->next) {
					...
					if (p->state == XICE_CHECK_SUCCEEDED ||
						p->state == XICE_CHECK_DISCOVERED) {
						xice_debug("Agent %p : restarting check %p as the nominated pair.", agent, p);
						p->nominated = TRUE;
	...
	}
	{
		...
		if (tick_counter++ % 50 == 0 || keep_timer_going != TRUE)
			xice_debug("Agent %p : timer tick #%u: %u frozen, %u in-progress, "
...
static gboolean priv_conn_check_tick_unlocked(gpointer pointer)
{
	...
	if (keep_timer_going != TRUE) {
		xice_debug("Agent %p : %s: stopping conncheck timer", agent, G_STRFUNC);
		for (i = agent->streams; i; i = i->next) {
			Stream *stream = i->data;
		...
		xice_debug("Agent %p : changing conncheck state to COMPLETED.", agent);
	}
...
static gboolean priv_conn_keepalive_retransmissions_tick(gpointer pointer)
{
	...
	case STUN_USAGE_TIMER_RETURN_TIMEOUT:
	{
		...
		if (pair->keepalive.agent->media_after_tick) {
			xice_debug("Agent %p : Keepalive conncheck timed out!! "
	...
	case STUN_USAGE_TIMER_RETURN_RETRANSMIT:
		/* Retransmit */
		xice_socket_send(pair->local->sockptr, &pair->remote->addr,
			stun_message_length(&pair->keepalive.stun_message),
			(gchar *)pair->keepalive.stun_buffer);

		xice_debug("Agent %p : Retransmitting keepalive conncheck",
 ...
 * @return will return FALSE when no more pending timers.
 */
static gboolean priv_conn_keepalive_tick_unlocked(XiceAgent *agent)
{
	GSList *i, *j, *k;
	int errors = 0;
	gboolean ret = FALSE;
		...
		for (j = stream->components; j; j = j->next) {
			...
			if (component->selected_pair.local != NULL) {
				...
				if (agent->compatibility == XICE_COMPATIBILITY_GOOGLE) {
					guint32 priority = xice_candidate_ice_priority_full(
						XICE_CANDIDATE_TYPE_PREF_PEER_REFLEXIVE, 1,
						p->local->component_id);
					uint8_t uname[XICE_STREAM_MAX_UNAME];
					size_t uname_len =
					...
					xice_address_to_string(&p->remote->addr, tmpbuf);
					xice_debug("Agent %p : Keepalive STUN-CC REQ to '%s:%u', "
						"socket=%u (c-id:%u), username='%s' (%d), "
						"password='%s' (%d), priority=%u.", agent,
						tmpbuf, xice_address_get_port(&p->remote->addr),
						xice_socket_get_fd((XiceSocket *)p->local->sockptr), component->id,
					...
					if (uname_len > 0) {
						...
						xice_debug("Agent %p: conncheck created %d - %p",
					...
					if (buf_len > 0) {
						xice_socket_send(p->local->sockptr, &p->remote->addr, buf_len,
							(gchar *)p->keepalive.stun_buffer);

						xice_debug("Agent %p : stun_bind_keepalive for pair %p res %d.",
							agent, p, (int)buf_len);
					}
					else {
						++errors;
	...
	for (i = agent->streams; i; i = i->next) {
		...
		for (j = stream->components; j; j = j->next) {
			Component *component = j->data;
			if (component->state < XICE_COMPONENT_STATE_READY &&
				...
				if (xice_address_set_from_string(&stun_server, agent->stun_server_ip)) {
					...
					for (k = component->local_candidates; k; k = k->next) {
						XiceCandidate *candidate = (XiceCandidate *)k->data;
						if (candidate->type == XICE_CANDIDATE_TYPE_HOST) {
							/* send the conncheck */
							xice_debug("Agent %p : resending STUN on %s to keep the "
								"candidate alive.", agent, candidate->foundation);
							xice_socket_send(candidate->sockptr, &stun_server,
	...
	if (errors) {
		xice_debug("Agent %p : %s: stopping keepalive timer", agent, G_STRFUNC);
		goto done;
	}
...
static gboolean priv_turn_allocate_refresh_retransmissions_tick(gpointer pointer)
{
	...
	case STUN_USAGE_TIMER_RETURN_RETRANSMIT:
		/* Retransmit */
		xice_socket_send(cand->xicesock, &cand->server,
...
static void priv_turn_allocate_refresh_tick_unlocked(CandidateRefresh *cand)
{
	...
	xice_debug("Agent %p : Sending allocate Refresh %d", cand->agent, buffer_len);
...
gboolean conn_check_schedule_next(XiceAgent *agent)
{
	...
	xice_debug("Agent %p : priv_conn_check_unfreeze_next returned %d", agent, res);

	if (agent->discovery_unsched_items > 0)
		xice_debug("Agent %p : WARN: starting conn checks before local candidate gathering is finished.", agent);

	/* step: call once imediately */
	res = priv_conn_check_tick_unlocked((gpointer)agent);
	xice_debug("Agent %p : priv_conn_check_tick_unlocked returned %d", agent, res);
	...
	if (agent->keepalive_timer_source == NULL) {
		agent->keepalive_timer_source = agent_timeout_add_with_context(agent, XICE_AGENT_TIMER_TR_DEFAULT, priv_conn_keepalive_tick, agent);
	}

	xice_debug("Agent %p : conn_check_schedule_next returning %d", agent, res);
	return res;
}
 ...
 */
static void priv_preprocess_conn_check_pending_data(XiceAgent *agent, Stream *stream, Component *component, CandidateCheckPair *pair)
{
	GSList *i;
	for (i = component->incoming_checks; i; i = i->next) {
		IncomingCheck *icheck = i->data;
		if (xice_address_equal(&icheck->from, &pair->remote->addr) &&
			icheck->local_socket == pair->local->sockptr) {
			xice_debug("Agent %p : Updating check %p with stored early-icheck %p, %p/%u/%u (agent/stream/component).", agent, pair, icheck, agent, stream->id, component->id);
		   ...
		   and section 7.2.1.5 */
			...
			for (k = component->incoming_checks; k; k = k->next) {
				...
				for (l = component->remote_candidates; l; l = l->next) {
					XiceCandidate *cand = l->data;
					if (xice_address_equal(&icheck->from, &cand->addr)) {
						match = TRUE;
						break;
					}
				}
				if (match != TRUE) {
					...
					XiceCandidate *local_candidate = NULL;
					XiceCandidate *remote_candidate = NULL;
					...
					if (agent->compatibility == XICE_COMPATIBILITY_GOOGLE ||
						...
						xice_debug("Agent %p: We have a peer-reflexive candidate in a "
						...
						for (m = component->remote_candidates;
							...
							for (n = component->local_candidates; n; n = n->next) {
								XiceCandidate *rcand = m->data;
								XiceCandidate *lcand = n->data;
					...
					if (agent->compatibility == XICE_COMPATIBILITY_GOOGLE &&
						...
						xice_debug("Agent %p : Username check failed. pending check has "
							"to wait to be processed", agent);
					}
					else {
						XiceCandidate *candidate;

						xice_debug("Agent %p : Discovered peer reflexive from early i-check",
							agent);
						candidate =
...
static GSList *priv_limit_conn_check_list_size(GSList *conncheck_list, guint upper_limit)
{
	...
	if (list_len > upper_limit) {
		xice_debug("Agent : Pruning candidates. Conncheck list has %d elements. "
 ...
 */
static gboolean priv_update_selected_pair(XiceAgent *agent, Component *component, CandidateCheckPair *pair)
{
	g_assert(component);
	g_assert(pair);
	if (pair->priority > component->selected_pair.priority) {
		xice_debug("Agent %p : changing SELECTED PAIR for component %u: %s:%s "
 ...
 * failed, mark that component's state to XICE_CHECK_FAILED.
 *
	...
	if (nominated > 0) {
		...
		if (priv_prune_pending_checks(stream, component->id) == 0) {
			agent_signal_component_state_change(agent, stream->id,
				component->id, XICE_COMPONENT_STATE_READY);
		}
	}
	xice_debug("Agent %p : conn.check list status: %u nominated, %u succeeded, c-id %u.", agent, nominated, succeeded, component->id);
}
...
static void priv_mark_pair_nominated(XiceAgent *agent, Stream *stream, Component *component, XiceCandidate *remotecand)
{
	...
	for (i = stream->conncheck_list; i; i = i->next) {
		...
		if (pair->remote == remotecand) {
			xice_debug("Agent %p : marking pair %p (%s) as nominated", agent, pair, pair->foundation);
			pair->nominated = TRUE;
			if (pair->state == XICE_CHECK_SUCCEEDED ||
				pair->state == XICE_CHECK_DISCOVERED)
 ...
 */
static void priv_add_new_check_pair(XiceAgent *agent, guint stream_id, Component *component, XiceCandidate *local, XiceCandidate *remote, XiceCheckState initial_state, gboolean use_candidate)
{
	...
	pair->local = local;
	pair->remote = remote;
	g_snprintf(pair->foundation, XICE_CANDIDATE_PAIR_MAX_FOUNDATION, "%s:%s", local->foundation, remote->foundation);

	pair->priority = agent_candidate_pair_priority(agent, local, remote);
	pair->state = initial_state;
	xice_debug("Agent %p : creating new pair %p state %d", agent, pair, initial_state);
	pair->nominated = use_candidate;
	pair->controlling = agent->controlling_mode;

	xice_debug("Agent %p : added a new conncheck %p with foundation of '%s' to list %u.", agent, pair, pair->foundation, stream_id);

	/* implement the hard upper limit for number of
	   checks (see sect 5.7.3 ICE ID-19): */
	if (agent->compatibility == XICE_COMPATIBILITY_RFC5245) {
		stream->conncheck_list =
			priv_limit_conn_check_list_size(stream->conncheck_list, agent->max_conn_checks);
	}
}

static gboolean priv_conn_check_add_for_candidate_pair(XiceAgent *agent, guint stream_id, Component *component, XiceCandidate *local, XiceCandidate *remote)
{
	gboolean ret = FALSE;
	...
	if ((agent->compatibility == XICE_COMPATIBILITY_RFC5245 ||
		...
		local->type == XICE_CANDIDATE_TYPE_SERVER_REFLEXIVE) {
		return FALSE;
	}
	...
	if (local->transport == remote->transport &&
		...
		priv_add_new_check_pair(agent, stream_id, component, local, remote, XICE_CHECK_FROZEN, FALSE);
		ret = TRUE;
		if (component->state < XICE_COMPONENT_STATE_CONNECTED) {
			agent_signal_component_state_change(agent,
				stream_id,
				component->id,
...
void conn_check_free(XiceAgent *agent)
{
	...
	for (i = agent->streams; i; i = i->next) {
		...
		xice_debug("Agent %p, freeing conncheck_list of stream %p", agent, stream);
 ...
 */
static
size_t priv_gen_username(XiceAgent *agent, guint component_id,
	gchar *remote, gchar *local, uint8_t *dest, guint dest_len)
{
	...
	if (remote_len > 0 && local_len > 0) {
		...
		else if (agent->compatibility == XICE_COMPATIBILITY_GOOGLE &&
 ...
 */
static
size_t priv_create_username(XiceAgent *agent, Stream *stream,
	guint component_id, XiceCandidate *remote, XiceCandidate *local,
	uint8_t *dest, guint dest_len, gboolean inbound)
{
 ...
 */
static
size_t priv_get_password(XiceAgent *agent, Stream *stream,
	XiceCandidate *remote, uint8_t **password)
{
	if (agent->compatibility == XICE_COMPATIBILITY_GOOGLE)
		return 0;
...
int conn_check_send(XiceAgent *agent, CandidateCheckPair *pair)
{
	...
	guint32 priority =
		xice_candidate_ice_priority_full(
			XICE_CANDIDATE_TYPE_PREF_PEER_REFLEXIVE,
			1,
	...
	if (agent->compatibility == XICE_COMPATIBILITY_MSN ||
		agent->compatibility == XICE_COMPATIBILITY_OC2007) {
		password = g_base64_decode((gchar *)password, &password_len);
	}
	...
	{
		gchar tmpbuf[INET6_ADDRSTRLEN];
		xice_address_to_string(&pair->remote->addr, tmpbuf);
		xice_debug("Agent %p : STUN-CC REQ to '%s:%u', socket=%u, pair=%s (c-id:%u), tie=%llu, username='%s' (%d), password='%s' (%d), priority=%u.", agent,
			tmpbuf,
			xice_address_get_port(&pair->remote->addr),
			xice_socket_get_fd((XiceSocket *)pair->local->sockptr),
	...
	if (uname_len > 0) {
		...
		xice_debug("Agent %p: conncheck created %d - %p", agent, buffer_len, pair->stun_message.buffer);
		...
		if (buffer_len > 0) {
			...
			xice_debug("Agent %p: buffer is empty, cancelling conncheck", agent);
			pair->stun_message.buffer = NULL;
			pair->stun_message.buffer_len = 0;
			return -1;
		}
		...
		xice_debug("Agent %p: no credentials found, cancelling conncheck", agent);
		pair->stun_message.buffer = NULL;
		pair->stun_message.buffer_len = 0;
		return -1;
	}
...
static guint priv_prune_pending_checks(Stream *stream, guint component_id)
{
	...
	xice_debug("Agent XXX: Finding highest priority for component %d",
		component_id);

	for (i = stream->conncheck_list; i; i = i->next) {
		CandidateCheckPair *p = i->data;
		if (p->component_id == component_id &&
			(p->state == XICE_CHECK_SUCCEEDED ||
				p->state == XICE_CHECK_DISCOVERED) &&
			p->nominated == TRUE) {
	...
	xice_debug("Agent XXX: Pruning pending checks. Highest nominated priority "
	...
	for (i = stream->conncheck_list; i; i = i->next) {
		...
		if (p->component_id == component_id) {
			if (p->state == XICE_CHECK_FROZEN ||
				p->state == XICE_CHECK_WAITING) {
				p->state = XICE_CHECK_CANCELLED;
				xice_debug("Agent XXX : pair %p state CANCELED", p);
			}
			...
			if (p->state == XICE_CHECK_IN_PROGRESS) {
				if (highest_nominated_priority != 0 &&
					...
					xice_debug("Agent XXX : pair %p kept IN_PROGRESS because priority %"
...
static gboolean priv_schedule_triggered_check(XiceAgent *agent, Stream *stream, Component *component, XiceSocket *local_socket, XiceCandidate *remote_cand, gboolean use_candidate)
{
	...
	for (i = stream->conncheck_list; i; i = i->next) {
		...
		if (p->component_id == component->id &&
			...
			xice_debug("Agent %p : Found a matching pair %p for triggered check.", agent, p);

			if (p->state == XICE_CHECK_WAITING ||
				p->state == XICE_CHECK_FROZEN)
				priv_conn_check_initiate(agent, p);
			else if (p->state == XICE_CHECK_IN_PROGRESS) {
				 ...
				 */
				xice_debug("Agent %p : check already in progress, "
				...
				p->state == XICE_CHECK_DISCOVERED) {
				xice_debug("Agent %p : Skipping triggered check, already completed..", agent);
			...
			else if (p->state == XICE_CHECK_FAILED) {
	...
	if (i) {
		xice_debug("Agent %p : Adding a triggered check to conn.check list (local=%p).", agent, local);
		priv_add_new_check_pair(agent, stream->id, component, local, remote_cand, XICE_CHECK_WAITING, use_candidate);
		return TRUE;
	}
	else {
		xice_debug("Agent %p : Didn't find a matching pair for triggered check (remote-cand=%p).", agent, remote_cand);
		return FALSE;
	}
}
 ...
 *
 * @pre (rcand == NULL || xice_address_equal(rcand->addr, toaddr) == TRUE)
 */
static void priv_reply_to_conn_check(XiceAgent *agent, Stream *stream, Component *component, XiceCandidate *rcand, const XiceAddress *toaddr, XiceSocket *socket, size_t  rbuf_len, uint8_t *rbuf, gboolean use_candidate)
{
	g_assert(rcand == NULL || xice_address_equal(&rcand->addr, toaddr) == TRUE);

	{
		gchar tmpbuf[INET6_ADDRSTRLEN];
		xice_address_to_string(toaddr, tmpbuf);
		xice_debug("Agent %p : STUN-CC RESP to '%s:%u', socket=%u, len=%u, cand=%p (c-id:%u), use-cand=%d.", agent,
			tmpbuf,
			xice_address_get_port(toaddr),
			xice_socket_get_fd(socket),
			(unsigned)rbuf_len,
			rcand, component->id,
			(int)use_candidate);
	}

	xice_socket_send(socket, toaddr, rbuf_len, (const gchar*)rbuf);
 ...
 */
static int priv_store_pending_check(XiceAgent *agent, Component *component,
	const XiceAddress *from, XiceSocket *socket, uint8_t *username,
	uint16_t username_len, uint32_t priority, gboolean use_candidate)
{
	IncomingCheck *icheck;
	xice_debug("Agent %p : Storing pending check.", agent);

	if (component->incoming_checks &&
		g_slist_length(component->incoming_checks) >=
		XICE_AGENT_MAX_REMOTE_CANDIDATES) {
		xice_debug("Agent %p : WARN: unable to store information for early incoming check.", agent);
		return -1;
	}
 ...
 */
static CandidateCheckPair *priv_add_peer_reflexive_pair(XiceAgent *agent, guint stream_id, guint component_id, XiceCandidate *local_cand, CandidateCheckPair *parent_pair)
{
	CandidateCheckPair *pair = g_slice_new0(CandidateCheckPair);
	Stream *stream = agent_find_stream(agent, stream_id);
	...
	pair->state = XICE_CHECK_DISCOVERED;
	xice_debug("Agent %p : pair %p state DISCOVERED", agent, pair);
	g_snprintf(pair->foundation, XICE_CANDIDATE_PAIR_MAX_FOUNDATION, "%s:%s",
		local_cand->foundation, parent_pair->remote->foundation);
	if (agent->controlling_mode == TRUE)
		pair->priority = xice_candidate_pair_priority(pair->local->priority,
			pair->remote->priority);
	else
		pair->priority = xice_candidate_pair_priority(pair->remote->priority,
			pair->local->priority);
	pair->nominated = FALSE;
	pair->controlling = agent->controlling_mode;
	xice_debug("Agent %p : added a new peer-discovered pair with foundation of '%s'.", agent, pair->foundation);
 ...
 */
static void priv_recalculate_pair_priorities(XiceAgent *agent)
{
	GSList *i, *j;
 ...
 */
static void priv_check_for_role_conflict(XiceAgent *agent, gboolean control)
{
	/* role conflict, change mode; wait for a new conn. check */
	if (control != agent->controlling_mode) {
		xice_debug("Agent %p : Role conflict, changing agent role to %d.", agent, control);
		...
		xice_debug("Agent %p : Role conflict, agent role already changed to %d.", agent, control);
}
 ...
 */
static CandidateCheckPair *priv_process_response_check_for_peer_reflexive(XiceAgent *agent, Stream *stream, Component *component, CandidateCheckPair *p, XiceSocket *sockptr, struct sockaddr *mapped_sockaddr, XiceCandidate *local_candidate, XiceCandidate *remote_candidate)
{
	...
	xice_address_set_from_sockaddr(&mapped, mapped_sockaddr);

	for (j = component->local_candidates; j; j = j->next) {
		XiceCandidate *cand = j->data;
		if (xice_address_equal(&mapped, &cand->addr)) {
	...
	if (local_cand_matches == TRUE) {
		/* note: this is same as "adding to VALID LIST" in the spec
		   text */
		p->state = XICE_CHECK_SUCCEEDED;
		xice_debug("Agent %p : conncheck %p SUCCEEDED.", agent, p);
		...
		xice_debug("Agent %p : conncheck %p FAILED, %p DISCOVERED.", agent, p, new_pair);
	}
 ...
 */
static gboolean priv_map_reply_to_conn_check_request(XiceAgent *agent, Stream *stream, Component *component, XiceSocket *sockptr, const XiceAddress *from, XiceCandidate *local_candidate, XiceCandidate *remote_candidate, StunMessage *resp)
{
		...
		if (p->stun_message.buffer) {
			...
			if (memcmp(discovery_id, response_id, sizeof(StunTransactionId)) == 0) {
				res = stun_usage_ice_conncheck_process(resp,
					...
					agent_to_ice_compatibility(agent));
				xice_debug("Agent %p : stun_bind_process/conncheck for %p res %d "
				...
				if (res == STUN_USAGE_ICE_RETURN_SUCCESS ||
					...
					xice_debug("Agent %p : conncheck %p MATCHED.", agent, p);
					p->stun_message.buffer = NULL;
					p->stun_message.buffer_len = 0;
					 ...
					 *       Cases") */
					if (xice_address_equal(from, &p->remote->addr) != TRUE) {
						gchar tmpbuf[INET6_ADDRSTRLEN];
						gchar tmpbuf2[INET6_ADDRSTRLEN];

						p->state = XICE_CHECK_FAILED;
						xice_debug("Agent %p : conncheck %p FAILED"
							" (mismatch of source address).", agent, p);
						xice_address_to_string(&p->remote->addr, tmpbuf);
						xice_address_to_string(from, tmpbuf2);
						xice_debug("Agent %p : '%s:%u' != '%s:%u'", agent,
							tmpbuf, xice_address_get_port(&p->remote->addr),
							tmpbuf2, xice_address_get_port(from));
					...
					if (res == STUN_USAGE_ICE_RETURN_NO_MAPPED_ADDRESS) {
						/* note: this is same as "adding to VALID LIST" in the spec
						   text */
						p->state = XICE_CHECK_SUCCEEDED;
						xice_debug("Agent %p : Mapped address not found."
					...
					if (component->state != XICE_COMPONENT_STATE_READY) {
						...
						agent_signal_component_state_change(agent,
							stream->id, component->id, XICE_COMPONENT_STATE_CONNECTED);
					}
					...
					xice_debug("Agent %p : conncheck %p ROLE CONFLICT, restarting", agent, p);
					...
					xice_debug("Agent %p : conncheck %p FAILED.", agent, p);
					p->stun_message.buffer = NULL;
					p->stun_message.buffer_len = 0;
					trans_found = TRUE;
				}
 ...
 * @return TRUE if a matching transaction is found
 */
static gboolean priv_map_reply_to_discovery_request(XiceAgent *agent, StunMessage *resp)
{
	struct sockaddr_storage sockaddr;
	socklen_t socklen = sizeof(sockaddr);
	...
	for (i = agent->discovery_list; i && trans_found != TRUE; i = i->next) {
		...
		if (d->type == XICE_CANDIDATE_TYPE_SERVER_REFLEXIVE &&
			...
			if (memcmp(discovery_id, response_id, sizeof(StunTransactionId)) == 0) {
				...
				xice_debug("Agent %p : stun_bind_process/disc for %p res %d.",
					agent, d, (int)res);

				if (res == STUN_USAGE_BIND_RETURN_ALTERNATE_SERVER) {
					/* handle alternate server */
					XiceAddress xiceaddr;
					xice_address_set_from_sockaddr(&xiceaddr,
...
priv_add_new_turn_refresh(CandidateDiscovery *cdisco, XiceCandidate *relay_cand,
	guint lifetime)
{
	CandidateRefresh *cand;
	XiceAgent *agent = cdisco->agent;

	cand = g_slice_new0(CandidateRefresh);
	agent->refresh_list = g_slist_append(agent->refresh_list, cand);

	cand->xicesock = cdisco->xicesock;
	...
	xice_debug("Agent %p : Adding new refresh candidate %p with timeout %d",
	...
	xice_debug("timer source is : %d", cand->timer_source);
 ...
 * @return TRUE if a matching transaction is found
 */
static gboolean priv_map_reply_to_relay_request(XiceAgent *agent, StunMessage *resp)
{
	struct sockaddr_storage sockaddr;
	socklen_t socklen = sizeof(sockaddr);
	struct sockaddr_storage alternate;
	...
	for (i = agent->discovery_list; i && trans_found != TRUE; i = i->next) {
		...
		if (d->type == XICE_CANDIDATE_TYPE_RELAYED &&
			d->stun_message.buffer) {
			stun_message_id(&d->stun_message, discovery_id);
			...
			if (memcmp(discovery_id, response_id, sizeof(StunTransactionId)) == 0) {
				...
				xice_debug("Agent %p : stun_turn_process/disc for %p res %d.",
					agent, d, (int)res);

				if (res == STUN_USAGE_TURN_RETURN_ALTERNATE_SERVER) {
					/* handle alternate server */
					xice_address_set_from_sockaddr(&d->server,
						(struct sockaddr *) &alternate);
					xice_address_set_from_sockaddr(&d->turn->server,
						(struct sockaddr *) &alternate);
					...
					if (relay_cand) {
						...
						if (agent->compatibility == XICE_COMPATIBILITY_OC2007 ||
							agent->compatibility == XICE_COMPATIBILITY_OC2007R2) {
							/* These data are needed on TURN socket when sending requests,
							 * but never reach xice_turn_socket_parse_recv() where it could
							 * be read directly, as the socket does not exist when allocate
							 * response arrives to _xice_agent_recv(). We must set them right
							 * after socket gets created in discovery_add_relay_candidate(),
							 * so we are doing it here. */
							xice_turn_socket_set_ms_realm(relay_cand->sockptr, &d->stun_message);
							xice_turn_socket_set_ms_connection_id(relay_cand->sockptr, resp);
						}
					}
 ...
 * @return TRUE if a matching transaction is found
 */
static gboolean priv_map_reply_to_relay_refresh(XiceAgent *agent, StunMessage *resp)
{
	uint32_t lifetime;
	GSList *i;
	StunUsageTurnReturn res;
	...
	for (i = agent->refresh_list; i && trans_found != TRUE; i = i->next) {
		...
		if (cand->stun_message.buffer) {
			...
			if (memcmp(refresh_id, response_id, sizeof(StunTransactionId)) == 0) {
				...
				xice_debug("Agent %p : stun_turn_refresh_process for %p res %d.",
					agent, cand, (int)res);
				if (res == STUN_USAGE_TURN_RETURN_RELAY_SUCCESS) {
					...
					/* check for unauthorized error response */
					if (cand->agent->compatibility == XICE_COMPATIBILITY_RFC5245 &&
						stun_message_get_class(resp) == STUN_ERROR &&
						stun_message_find_error(resp, &code) ==
						STUN_MESSAGE_RETURN_SUCCESS &&
						recv_realm != NULL && recv_realm_len > 0) {
...
static gboolean priv_map_reply_to_keepalive_conncheck(XiceAgent *agent,
	Component *component, StunMessage *resp)
{
	...
	if (component->selected_pair.keepalive.stun_message.buffer) {
		...
		if (memcmp(conncheck_id, response_id, sizeof(StunTransactionId)) == 0) {
			xice_debug("Agent %p : Keepalive for selected pair received.",
				agent);
	...
	uint8_t **password, size_t *password_len, void *user_data)
{
	...
	gboolean msn_msoc_xice_compatibility =
		data->agent->compatibility == XICE_COMPATIBILITY_MSN ||
		data->agent->compatibility == XICE_COMPATIBILITY_OC2007;

	if (data->agent->compatibility == XICE_COMPATIBILITY_OC2007 &&
	...
	for (; i; i = i->next) {
		...
		if (ufrag && msn_msoc_xice_compatibility)
		...
		if (ufrag_len > 0 && username_len >= ufrag_len &&
			...
			if (pass) {
				...
				*password_len = strlen(pass);
				...
				if (msn_msoc_xice_compatibility) {
 ...
 */
gboolean conn_check_handle_inbound_stun(XiceAgent *agent, Stream *stream,
	Component *component, XiceSocket *socket, const XiceAddress *from,
	gchar *buf, guint len)
{
	struct sockaddr_storage sockaddr;
	...
	GSList *i, *j;
	XiceCandidate *remote_candidate = NULL;
	XiceCandidate *remote_candidate2 = NULL;
	XiceCandidate *local_candidate = NULL;
	gboolean discovery_msg = FALSE;

	xice_address_copy_to_sockaddr(from, (struct sockaddr *) &sockaddr);
...
#ifndef NDEBUG
	{
		...
		xice_debug("Agent %p: inbound STUN packet for %u/%u (stream/component) from [%s]:%u (%u octets) :",
			agent, stream->id, component->id, tmpbuf, xice_address_get_port(from), len);
	}
#endif
	...
	if (valid == STUN_VALIDATION_BAD_REQUEST ||
		...
		for (i = agent->refresh_list; i; i = i->next) {
			...
			xice_debug("Comparing %p to %p, %p to %p and %p and %p to %p", r->stream,
				stream, r->component, component, r->xicesock, r->relay_socket, socket);
			if (r->stream == stream && r->component == component &&
				(r->xicesock == socket || r->relay_socket == socket)) {
				valid = stun_agent_validate(&r->stun_agent, &req,
					(uint8_t *)buf, len, conncheck_stun_validater, &validater_data);
				xice_debug("Validating gave %d", valid);
				if (valid == STUN_VALIDATION_UNMATCHED_RESPONSE)
					continue;
				discovery_msg = TRUE;
				break;
	...
	{
		xice_debug("Agent %p : Incorrectly multiplexed STUN message ignored.",
			agent);
		return FALSE;
	}

	if (valid == STUN_VALIDATION_UNKNOWN_REQUEST_ATTRIBUTE) {
		xice_debug("Agent %p : Unknown mandatory attributes in message.", agent);
		...
		if (agent->compatibility != XICE_COMPATIBILITY_MSN &&
			...
			if (rbuf_len != 0)
				xice_socket_send(socket, from, rbuf_len, (const gchar*)rbuf);
		}
		return TRUE;
	}

	if (valid == STUN_VALIDATION_UNAUTHORIZED) {
		xice_debug("Agent %p : Integrity check failed.", agent);
		...
		if (stun_agent_init_error(&agent->stun_agent, &msg, rbuf, rbuf_len,
			...
			if (rbuf_len > 0 && agent->compatibility != XICE_COMPATIBILITY_MSN &&
				agent->compatibility != XICE_COMPATIBILITY_OC2007)
				xice_socket_send(socket, from, rbuf_len, (const gchar*)rbuf);
		}
		return TRUE;
	...
	for (i = component->remote_candidates; i; i = i->next) {
		XiceCandidate *cand = i->data;
		if (xice_address_equal(from, &cand->addr)) {
	...
	if (agent->compatibility == XICE_COMPATIBILITY_GOOGLE ||
		...
		for (i = component->remote_candidates;
			...
			for (j = component->local_candidates; j; j = j->next) {
				...
				XiceCandidate *rcand = i->data;
				XiceCandidate *lcand = j->data;
	...
	if (component->remote_candidates &&
		...
		xice_debug("Agent %p : Username check failed.", agent);
		return TRUE;
	}

	if (valid != STUN_VALIDATION_SUCCESS) {
		xice_debug("Agent %p : STUN message is unsuccessfull %d, ignoring", agent, valid);
		return FALSE;
	}
	...
	if (stun_message_get_class(&req) == STUN_REQUEST) {
		if (agent->compatibility == XICE_COMPATIBILITY_MSN
			...
			if (local_candidate && remote_candidate2) {
				...
				xice_debug("Agent %p : received MSN incoming check from unknown remote candidate. "
		...
		if (res == STUN_USAGE_ICE_RETURN_SUCCESS ||
			...
			if (component->remote_candidates && remote_candidate == NULL) {
				xice_debug("Agent %p : No matching remote candidate for incoming check ->"
			...
			xice_debug("Agent %p : Invalid STUN packet, ignoring... %s",
		...
		if (trans_found != TRUE)
			xice_debug("Agent %p : Unable to match to an existing transaction, "
