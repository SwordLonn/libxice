...
void refresh_free_item (gpointer data, gpointer user_data)
{
  ...
  if (buffer_len > 0) {
    ...
    if (!xice_socket_is_reliable (cand->xicesock)) {
      xice_socket_send (cand->xicesock, &cand->server,
          buffer_len, (gchar *)cand->stun_buffer);
    }
 ...
 * Candidates" (ID-19).
 */
static gboolean priv_add_local_candidate_pruned (XiceAgent *agent, guint stream_id, Component *component, XiceCandidate *candidate)
{
  GSList *i;

  for (i = component->local_candidates; i ; i = i->next) {
    XiceCandidate *c = i->data;

    if (xice_address_equal (&c->base_addr, &candidate->base_addr) &&
	xice_address_equal (&c->addr, &candidate->addr)) {
      xice_debug ("Candidate %p (component-id %u) redundant, ignoring.", candidate, component->id);
      return FALSE;
    }
  }
...
static guint priv_highest_remote_foundation (Component *component)
{
  GSList *i;
  guint highest = 1;
  gchar foundation[XICE_CANDIDATE_MAX_FOUNDATION];

  for (highest = 1;; highest++) {
    gboolean taken = FALSE;

    g_snprintf (foundation, XICE_CANDIDATE_MAX_FOUNDATION, "%u", highest);
 ...
 */
static void priv_assign_foundation (XiceAgent *agent, XiceCandidate *candidate)
{
  GSList *i, *j, *k;
  ...
  for (i = agent->streams; i; i = i->next) {
	...
	if (candidate->type == n->type &&
	    ...
	    xice_address_equal (&candidate->base_addr, &temp) &&
            !(agent->compatibility == XICE_COMPATIBILITY_GOOGLE &&
                n->type == XICE_CANDIDATE_TYPE_RELAYED)) {
  ...
  g_snprintf (candidate->foundation, XICE_CANDIDATE_MAX_FOUNDATION,
      "%u", agent->next_candidate_id++);
}

static void priv_assign_remote_foundation (XiceAgent *agent, XiceCandidate *candidate)
{
  GSList *i, *j, *k;
  ...
  for (i = agent->streams; i; i = i->next) {
	...
	if (candidate->type == n->type &&
            candidate->stream_id == n->stream_id &&
	    xice_address_equal (&candidate->addr, &temp)) {
	  /* note: currently only one STUN/TURN server per stream at a
	   *       time is supported, so there is no need to check
	   *       for candidates that would otherwise share the
	   *       foundation, but have different STUN/TURN servers */
...
static
void priv_generate_candidate_credentials (XiceAgent *agent,
    XiceCandidate *candidate)
{
  ...
  if (agent->compatibility == XICE_COMPATIBILITY_MSN ||
    ...
    xice_rng_generate_bytes (agent->rng, 32, (gchar *)username);
    xice_rng_generate_bytes (agent->rng, 16, (gchar *)password);
    ...
    xice_rng_generate_bytes_print (agent->rng, 16, (gchar *)username);
 ...
 */
XiceCandidate *discovery_add_local_host_candidate (
  XiceAgent *agent,
  guint stream_id,
  guint component_id,
  XiceAddress *address)
{
  ...
  Component *component;
  Stream *stream;
  XiceSocket *udp_socket = NULL;

  if (!agent_find_component (agent, stream_id, component_id, &stream, &component))
    return NULL;

  candidate = xice_candidate_new (XICE_CANDIDATE_TYPE_HOST);
  candidate->stream_id = stream_id;
  candidate->component_id = component_id;
  candidate->addr = *address;
  candidate->base_addr = *address;
  if (agent->compatibility == XICE_COMPATIBILITY_GOOGLE) {
    candidate->priority = xice_candidate_jingle_priority (candidate);
  } else if (agent->compatibility == XICE_COMPATIBILITY_MSN ||
             agent->compatibility == XICE_COMPATIBILITY_OC2007)  {
    candidate->priority = xice_candidate_msn_priority (candidate);
  } else {
    candidate->priority = xice_candidate_ice_priority (candidate);
  }
  ...
  XiceSocket *base_socket)
{
  ...
  candidate = xice_candidate_new (XICE_CANDIDATE_TYPE_SERVER_REFLEXIVE);
  ...
  if (agent->compatibility == XICE_COMPATIBILITY_GOOGLE) {
    ...
    candidate->priority =  xice_candidate_ice_priority_full
        (XICE_CANDIDATE_TYPE_PREF_SERVER_REFLEXIVE, 0, component_id);
  }
  candidate->stream_id = stream_id;
  candidate->component_id = component_id;
  candidate->addr = *address;
  ...
  if (result) {
    ...
    xice_candidate_free (candidate), candidate = NULL;
  }
  ...
  TurnServer *turn)
{
  ...
  Component *component;
  Stream *stream;
  XiceSocket *relay_socket = NULL;

  if (!agent_find_component (agent, stream_id, component_id, &stream, &component))
    return NULL;

  candidate = xice_candidate_new (XICE_CANDIDATE_TYPE_RELAYED);
  ...
  if (agent->compatibility == XICE_COMPATIBILITY_GOOGLE) {
    ...
    candidate->priority =  xice_candidate_ice_priority_full
        (XICE_CANDIDATE_TYPE_PREF_RELAYED, 0, component_id);
  }
  candidate->stream_id = stream_id;
  candidate->component_id = component_id;
  candidate->addr = *address;
  ...
  /* step: link to the base candidate+socket */
  relay_socket = xice_turn_socket_new (agent->main_context, address,
  ...
  XiceCandidate *remote)
{
  ...
  candidate = xice_candidate_new (XICE_CANDIDATE_TYPE_PEER_REFLEXIVE);
  ...
  if (agent->compatibility == XICE_COMPATIBILITY_GOOGLE) {
    ...
    candidate->priority = xice_candidate_ice_priority_full
        (XICE_CANDIDATE_TYPE_PREF_PEER_REFLEXIVE, 0, component_id);
  }
  candidate->stream_id = stream_id;
  candidate->component_id = component_id;
  candidate->addr = *address;
 ...
 * @return pointer to the created candidate, or NULL on error
 */
XiceCandidate *discovery_learn_remote_peer_reflexive_candidate (
  XiceAgent *agent,
  Stream *stream,
  Component *component,
  guint32 priority,
  ...
  XiceCandidate *remote)
{
  ...
  if (priority != 0) {
    ...
    candidate->priority = xice_candidate_ice_priority_full
        (XICE_CANDIDATE_TYPE_PREF_PEER_REFLEXIVE, 0, component->id);
  }
...
static gboolean priv_discovery_tick_unlocked (gpointer pointer)
{
...

  {
    ...
    if (tick_counter++ % 50 == 0)
      xice_debug ("Agent %p : discovery tick #%d with list %p (1)", agent, tick_counter, agent->discovery_list);
  }
  ...
  for (i = agent->discovery_list; i ; i = i->next) {
	...
	--agent->discovery_unsched_items;

      {
        ...
        xice_address_to_string (&cand->server, tmpbuf);
        xice_debug ("Agent %p : discovery - scheduling cand type %u addr %s.\n",
            agent, cand->type, tmpbuf);
      }
      if (xice_address_is_valid (&cand->server) &&
          (cand->type == XICE_CANDIDATE_TYPE_SERVER_REFLEXIVE ||
              cand->type == XICE_CANDIDATE_TYPE_RELAYED)) {
	...
	agent_signal_component_state_change (agent,
        ...
        if (cand->type == XICE_CANDIDATE_TYPE_SERVER_REFLEXIVE) {
          buffer_len = stun_usage_bind_create (&cand->stun_agent,
              &cand->stun_message, cand->stun_buffer, sizeof(cand->stun_buffer));
        } else if (cand->type == XICE_CANDIDATE_TYPE_RELAYED) {
	...
	xice_debug ("Agent %p : STUN discovery was cancelled, marking discovery done.", agent);
	cand->done = TRUE;
      }
          ...
          case STUN_USAGE_TIMER_RETURN_TIMEOUT:
            {
              ...
              cand->stun_message.buffer = NULL;
              cand->stun_message.buffer_len = 0;
              xice_debug ("Agent %p : bind discovery timed out, aborting discovery item.", agent);
              break;
            }
  ...
  if (not_done == 0) {
    xice_debug ("Agent %p : Candidate gathering FINISHED, stopping discovery timer.", agent);
