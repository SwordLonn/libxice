...
#ifndef _XICE_AGENT_PRIV_H
#define _XICE_AGENT_PRIV_H
 ...
 *      to session bandwidth -> this is not yet implemented in XICE */

#define XICE_AGENT_TIMER_TA_DEFAULT 20      /* timer Ta, msecs (impl. defined) */
#define XICE_AGENT_TIMER_TR_DEFAULT 25000   /* timer Tr, msecs (impl. defined) */
#define XICE_AGENT_TIMER_TR_MIN     15000   /* timer Tr, msecs (ICE ID-19) */
#define XICE_AGENT_MAX_CONNECTIVITY_CHECKS_DEFAULT 100 /* see spec 5.7.3 (ID-19) */
...
struct _XiceAgent
{
  ...
  XiceProxyType proxy_type;       /* property: Proxy type */
  ...
  XiceTimer *discovery_timer_source;
  XiceTimer *conncheck_timer_source;
  XiceTimer *keepalive_timer_source;

  GSList *refresh_list;         /* list of CandidateRefresh items */
  guint64 tie_breaker;            /* tie breaker (ICE sect 5.2
				     "Determining Role" ID-19) */
  XiceCompatibility compatibility; /* property: Compatibility mode */
...
StunUsageIceCompatibility agent_to_ice_compatibility (XiceAgent *agent);
StunUsageTurnCompatibility agent_to_turn_compatibility (XiceAgent *agent);
XiceTurnSocketCompatibility agent_to_turn_socket_compatibility (XiceAgent *agent);
