...
GList *
xice_interfaces_get_local_interfaces (void)
{
  ...
  for (ifa = results; ifa; ifa = ifa->ifa_next) {
    ...
    if (ifa->ifa_addr->sa_family == AF_INET || ifa->ifa_addr->sa_family == AF_INET6) {
      xice_debug ("Found interface : %s", ifa->ifa_name);
...
xice_interfaces_get_local_interfaces (void)
{
  ...
  if ((sockfd = socket (AF_INET, SOCK_DGRAM, IPPROTO_IP)) < 0) {
    xice_debug ("error : Cannot open socket to retreive interface list");
    return NULL;
  }
  ...
  do {
    ...
    if (NULL == (ifc.ifc_req = realloc (ifc.ifc_req, size))) {
      xice_debug ("Error : Out of memory while allocation interface"
  ...
  for (ifr = ifc.ifc_req;
       ...
       ++ifr) {
    xice_debug ("Found interface : %s", ifr->ifr_name);
...
static gboolean
xice_interfaces_is_private_ip (const struct sockaddr *sa)
{
...
GList *
xice_interfaces_get_local_ips (gboolean include_loopback)
{
  GList *ips = NULL;
  struct ifaddrs *ifa, *results;
  ...
  for (ifa = results; ifa; ifa = ifa->ifa_next) {
    ...
    xice_debug ("Interface:  %s", ifa->ifa_name);
    xice_debug ("IP Address: %s", addr_as_string);
    if ((ifa->ifa_flags & IFF_LOOPBACK) == IFF_LOOPBACK) {
      if (include_loopback)
        loopbacks = g_list_append (loopbacks, g_strdup (addr_as_string));
      else
        xice_debug ("Ignoring loopback interface");
    } else {
      if (xice_interfaces_is_private_ip (ifa->ifa_addr))
...
xice_interfaces_get_local_ips (gboolean include_loopback)
{
  ...
  for (ifr = ifc.ifc_req;
    ...
    if (ioctl (sockfd, SIOCGIFFLAGS, ifr)) {
      xice_debug ("Error : Unable to get IP information for interface %s."
          " Skipping...", ifr->ifr_name);
      continue;  /* failed to get flags, skip it */
    }
    sa = (struct sockaddr_in *) &ifr->ifr_addr;
    xice_debug ("Interface:  %s", ifr->ifr_name);
    xice_debug ("IP Address: %s", inet_ntoa (sa->sin_addr));
    if ((ifr->ifr_flags & IFF_LOOPBACK) == IFF_LOOPBACK){
      ...
      if (xice_interfaces_is_private_ip (sa)) {
...
gchar *
xice_interfaces_get_ip_for_interface (gchar *interface_name)
{
  struct ifreq ifr;
  struct sockaddr_in *sa;
  gint sockfd;
  ...
  close (sockfd);
  sa = (struct sockaddr_in *) &ifr.ifr_addr;
  xice_debug ("Address for %s: %s", interface_name, inet_ntoa (sa->sin_addr));
 ...
 */
SOCKET xice_interfaces_get_WSA_socket ()
{
  ...
  if (started_wsa_engine == FALSE) {
    ...
    if ( err != 0 ) {
      xice_debug ("Error : Could not start the winsocket engine");
  ...
  if ((sock = socket (AF_INET, SOCK_DGRAM, 0)) == INVALID_SOCKET) {
    xice_debug ("Error : Could not open socket to retreive interface list,"
...
GList * xice_interfaces_get_local_interfaces ()
{
  ULONG size = 0;
...
GList * xice_interfaces_get_local_ips (gboolean include_loopback)
{
  ULONG size = 0;
  DWORD pref = 0;
...
gchar * xice_interfaces_get_ip_for_interface (gchar *interface_name)
{
  ULONG size = 0;
  PMIB_IFTABLE if_table;
  gchar * ret = NULL;
