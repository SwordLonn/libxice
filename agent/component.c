...
component_new (guint id)
{
  ...
  component->id = id;
  component->state = XICE_COMPONENT_STATE_DISCONNECTED;
...
component_free (Component *cmp)
{
  ...
  if (cmp->restart_candidate)
    xice_candidate_free (cmp->restart_candidate),
...
component_find_pair (Component *cmp, XiceAgent *agent, const gchar *lfoundation, const gchar *rfoundation, CandidatePair *pair)
{
  ...
  for (i = cmp->local_candidates; i; i = i->next) {
    XiceCandidate *candidate = i->data;
    if (strncmp (candidate->foundation, lfoundation, XICE_CANDIDATE_MAX_FOUNDATION) == 0) {
  ...
  for (i = cmp->remote_candidates; i; i = i->next) {
    XiceCandidate *candidate = i->data;
    if (strncmp (candidate->foundation, rfoundation, XICE_CANDIDATE_MAX_FOUNDATION) == 0) {
...
void component_update_selected_pair (Component *component, const CandidatePair *pair)
{
  g_assert (component);
  g_assert (pair);
  xice_debug ("setting SELECTED PAIR for component %u: %s:%s (prio:%"
...
component_find_remote_candidate (const Component *component, const XiceAddress *addr, XiceCandidateTransport transport)
{
  ...
  for (i = component->remote_candidates; i; i = i->next) {
    ...
    if (xice_address_equal(&candidate->addr, addr) &&
    ...
    XiceCandidate *candidate)
{
  ...
  if (!remote) {
    remote = xice_candidate_copy (candidate);
