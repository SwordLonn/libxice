 ...
 * @stability: Unstable
 *
 * <para>Libxice can output a lot of information when debug messages are enabled.
 * This can significantly help track down problems and/or understand what
 * it's doing.</para>
 *
 * <para>You can enable/disable the debug messages by calling xice_debug_enable()
 * or xice_debug_disable() and choosing whether you want only ICE debug messages
 * or also stun debug messages.</para>
 *
 * <para>By default, the debug messages are disabled, unless the environment
 * variable XICE_DEBUG is set, in which case, it must contain a comma separated
 * list of flags specifying which debug to enable.</para>
 * <para> The currently available flags are "xice", "stun", "pseudotcp",
 ...
 * xice_debug_init:
 *
 * Initialize the debugging system. Uses the XICE_DEBUG environment variable
 * to set the appropriate debugging flags
 */
void xice_debug_init (void);

/**
 * xice_debug_enable:
 * @with_stun: Also enable stun debugging messages
 *
 * Enables libxice debug output to the terminal
 */
void xice_debug_enable (gboolean with_stun);

/**
 * xice_debug_disable:
 * @with_stun: Also disable stun debugging messages
 *
 * Disables libxice debug output to the terminal
 */
void xice_debug_disable (gboolean with_stun);

void xice_debug (const char *fmt, ...);
