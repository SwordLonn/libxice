...
XICEAPI_EXPORT XiceAddress *
xice_address_new (void)
{
  XiceAddress *addr = g_slice_new0 (XiceAddress);
...
xice_address_set_from_string (XiceAddress *addr, const gchar *str)
{
  ...
  if (inet_pton (AF_INET, str, &a.ipv4) > 0)
      xice_address_set_ipv4 (addr, ntohl (a.ipv4.s_addr));
  else if (inet_pton (AF_INET6, str, &a.ipv6) > 0)
      xice_address_set_ipv6 (addr, a.ipv6.s6_addr);
  else
...
XICEAPI_EXPORT void
xice_address_set_from_sockaddr (XiceAddress *addr,
...
XICEAPI_EXPORT void
xice_address_copy_to_sockaddr (const XiceAddress *addr,
                               struct sockaddr *sa)
{
...
XICEAPI_EXPORT XiceAddress *
xice_address_dup (const XiceAddress *a)
{
  XiceAddress *dup = g_slice_new0 (XiceAddress);
...
XICEAPI_EXPORT gboolean
xice_address_is_private (const XiceAddress *a)
{
  switch (a->s.addr.sa_family)
    {
    case AF_INET:
...
XICEAPI_EXPORT gboolean
xice_address_is_valid (const XiceAddress *a)
{
 switch (a->s.addr.sa_family)
    {
    case AF_INET:
