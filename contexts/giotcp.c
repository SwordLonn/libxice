...
static void add_to_be_sent (XiceSocket *sock, const gchar *buf, guint len,
    gboolean head);
...
attach_socket(
	XiceSocket *socket)
{
    TcpPriv *priv = (TcpPriv*)socket->priv;
...
socket_recv (XiceSocket *sock, XiceAddress *from, guint len, gchar *buf)
{
  ...
  ret = g_socket_receive ((GSocket*)sock->fileno, buf, len, NULL, &gerr);
    ...
    guint len, const gchar *buf)
{
  ...
  if (g_queue_is_empty (&priv->send_queue)) {
    ret = g_socket_send ((GSocket*)sock->fileno, buf, len, NULL, &gerr);
    if (ret < 0) {
      if(g_error_matches (gerr, G_IO_ERROR, G_IO_ERROR_WOULD_BLOCK)
  ...
  gpointer data)
{
  ...
  while ((tbs = g_queue_pop_head (&priv->send_queue)) != NULL) {
    ...
    if(condition & G_IO_HUP) {
      ...
      ret = g_socket_send ((GSocket*)sock->fileno, tbs->buf, tbs->length, NULL, &gerr);
    }
...
static void
add_to_be_sent (XiceSocket *sock, const gchar *buf, guint len, gboolean head)
{
  TcpPriv *priv = sock->priv;
  struct to_be_sent *tbs = NULL;
  ...
  if (priv->io_source == NULL) {
    priv->io_source = g_socket_create_source((GSocket*)sock->fileno, G_IO_OUT, NULL);
