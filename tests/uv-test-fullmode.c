...
#if USE_PROXY
#define PROXY_TYPE XICE_PROXY_TYPE_SOCKS5
#else
#define PROXY_TYPE XICE_PROXY_TYPE_NONE
#endif

#if TEST_GOOGLE
#define XICE_COMPATIBILITY XICE_COMPATIBILITY_GOOGLE
...
#define TURN_TYPE XICE_RELAY_TYPE_TURN_TLS

#endif

#else
#define XICE_COMPATIBILITY XICE_COMPATIBILITY_RFC5245
#if USE_LOOPBACK
...
#define TURN_TYPE XICE_RELAY_TYPE_TURN_TCP
#else
...
#define TURN_TYPE XICE_RELAY_TYPE_TURN_UDP
#endif

#endif


static XiceComponentState global_lagent_state[2] = { XICE_COMPONENT_STATE_LAST, XICE_COMPONENT_STATE_LAST };
static XiceComponentState global_ragent_state[2] = { XICE_COMPONENT_STATE_LAST, XICE_COMPONENT_STATE_LAST };
...
static void cb_writable (XiceAgent*agent, guint stream_id, guint component_id)
{
...
static void set_candidates (XiceAgent *from, guint from_stream,
    XiceAgent *to, guint to_stream, guint component, gboolean remove_non_relay)
{
  GSList *cands = NULL, *i;

  cands = xice_agent_get_local_candidates (from, from_stream, component);
  if (remove_non_relay) {
  restart:
    for (i = cands; i; i = i->next) {
      ...
      if (cand->type != XICE_CANDIDATE_TYPE_RELAYED) {
        ...
        xice_candidate_free (cand);
        goto restart;
      }
    }
  }
  xice_agent_set_remote_candidates (to, to_stream, component, cands);
...
static void set_credentials (XiceAgent *lagent, guint lstream,
    XiceAgent *ragent, guint rstream)
{
  gchar *ufrag = NULL, *password = NULL;

  xice_agent_get_local_credentials(lagent, lstream, &ufrag, &password);
  xice_agent_set_remote_credentials (ragent, rstream, ufrag, password);
  g_free (ufrag);
  g_free (password);
  xice_agent_get_local_credentials(ragent, rstream, &ufrag, &password);
  xice_agent_set_remote_credentials (lagent, lstream, ufrag, password);
  g_free (ufrag);
  g_free (password);
}

static int run_full_test (XiceAgent *lagent, XiceAgent *ragent, XiceAddress *baseaddr, guint ready, guint failed)
{
  guint ls_id, rs_id;
  gint ret;
  ...
  g_assert (rs_id > 0);
#if USE_TURN
  xice_agent_set_relay_info(lagent, ls_id, 1,
      TURN_IP, TURN_PORT, TURN_USER, TURN_PASS, TURN_TYPE);
  xice_agent_set_relay_info(lagent, ls_id, 2,
      TURN_IP, TURN_PORT, TURN_USER, TURN_PASS, TURN_TYPE);
  xice_agent_set_relay_info(ragent, rs_id, 1,
      TURN_IP, TURN_PORT, TURN_USER2, TURN_PASS2, TURN_TYPE);
  xice_agent_set_relay_info(ragent, rs_id, 2,
      TURN_IP, TURN_PORT, TURN_USER2, TURN_PASS2, TURN_TYPE);
#endif


  /* Gather candidates and test xice_agent_set_port_range */
  xice_agent_set_port_range (lagent, ls_id, 1, 10000, 10000);
  xice_agent_set_port_range (lagent, ls_id, 2, 10001, 10001);
  xice_agent_set_port_range (ragent, rs_id, 1, 12345, 12345);
  xice_agent_set_port_range (ragent, rs_id, 2, 10000, 10001);
  g_assert (xice_agent_gather_candidates (lagent, ls_id) == TRUE);
  g_assert (xice_agent_gather_candidates (ragent, rs_id) == FALSE);
  g_assert (xice_agent_get_local_candidates (ragent, rs_id, 1) == NULL);
  g_assert (xice_agent_get_local_candidates (ragent, rs_id, 2) == NULL);
  xice_agent_set_port_range (ragent, rs_id, 2, 10002, 10002);
  g_assert (xice_agent_gather_candidates (ragent, rs_id) == TRUE);

#ifdef USE_LOOPBACK
  {
    GSList *cands = NULL, *i;
    XiceCandidate *cand = NULL;

    cands = xice_agent_get_local_candidates (lagent, ls_id, 1);
    g_assert (g_slist_length (cands) == 1);
    cand = cands->data;
    g_assert (cand->type == XICE_CANDIDATE_TYPE_HOST);
    g_assert (xice_address_get_port (&cand->addr) == 10000);
    for (i = cands; i; i = i->next)
      xice_candidate_free ((XiceCandidate *) i->data);
    g_slist_free (cands);

    cands = xice_agent_get_local_candidates (lagent, ls_id, 2);
    g_assert (g_slist_length (cands) == 1);
    cand = cands->data;
    g_assert (cand->type == XICE_CANDIDATE_TYPE_HOST);
    g_assert (xice_address_get_port (&cand->addr) == 10001);
    for (i = cands; i; i = i->next)
      xice_candidate_free ((XiceCandidate *) i->data);
    g_slist_free (cands);

    cands = xice_agent_get_local_candidates (ragent, rs_id, 1);
    g_assert (g_slist_length (cands) == 1);
    cand = cands->data;
    g_assert (cand->type == XICE_CANDIDATE_TYPE_HOST);
    g_assert (xice_address_get_port (&cand->addr) == 12345);
    for (i = cands; i; i = i->next)
      xice_candidate_free ((XiceCandidate *) i->data);
    g_slist_free (cands);

    cands = xice_agent_get_local_candidates (ragent, rs_id, 2);
    g_assert (g_slist_length (cands) == 1);
    cand = cands->data;
    g_assert (cand->type == XICE_CANDIDATE_TYPE_HOST);
    g_assert (xice_address_get_port (&cand->addr) == 10002);
    for (i = cands; i; i = i->next)
      xice_candidate_free ((XiceCandidate *) i->data);
    g_slist_free (cands);
  ...
  set_candidates (ragent, rs_id, lagent, ls_id, XICE_COMPONENT_TYPE_RTP, USE_TURN);
  set_candidates (ragent, rs_id, lagent, ls_id, XICE_COMPONENT_TYPE_RTCP, USE_TURN);
  set_candidates (lagent, ls_id, ragent, rs_id, XICE_COMPONENT_TYPE_RTP, USE_TURN);
  set_candidates (lagent, ls_id, ragent, rs_id, XICE_COMPONENT_TYPE_RTCP, USE_TURN);
  ...
  ret = xice_agent_send (lagent, ls_id, 1, 16, "1234567812345678");
  if (ret == -1)
  {
 ...
 */
static int run_full_test_delayed_answer (XiceAgent *lagent, XiceAgent *ragent, XiceAddress *baseaddr, guint ready, guint failed)
{
  guint ls_id, rs_id;
  gint ret;
  ...
  set_candidates (lagent, ls_id, ragent, rs_id, XICE_COMPONENT_TYPE_RTP, FALSE);
  set_candidates (lagent, ls_id, ragent, rs_id, XICE_COMPONENT_TYPE_RTCP, FALSE);
  ...
  set_candidates (ragent, rs_id, lagent, ls_id, XICE_COMPONENT_TYPE_RTP, FALSE);
  set_candidates (ragent, rs_id, lagent, ls_id, XICE_COMPONENT_TYPE_RTCP, FALSE);
...
static int run_full_test_wrong_password (XiceAgent *lagent, XiceAgent *ragent, XiceAddress *baseaddr)
{
  guint ls_id, rs_id;
  ...
  set_credentials (lagent, ls_id, ragent, rs_id);
  xice_agent_set_remote_credentials (ragent, rs_id, "wrong", "password");
  xice_agent_set_remote_credentials (lagent, ls_id, "wrong2", "password2");
...
static int run_full_test_control_conflict (XiceAgent *lagent, XiceAgent *ragent, XiceAddress *baseaddr, gboolean role)
{
  guint ls_id, rs_id;
...
int main (void)
{
  ...
  /* step: create the agents L and R */
#if USE_RELIABLE
  lagent = xice_agent_new_reliable (lcontext,
      XICE_COMPATIBILITY);
  ragent = xice_agent_new_reliable (rcontext,
      XICE_COMPATIBILITY);
#else
  lagent = xice_agent_new (g_main_loop_get_context (global_mainloop),
      XICE_COMPATIBILITY);
  ragent = xice_agent_new (g_main_loop_get_context (global_mainloop),
      XICE_COMPATIBILITY);
#endif

  xice_agent_set_software (lagent, "Test-fullmode, Left Agent");
  xice_agent_set_software (ragent, "Test-fullmode, Right Agent");

  /* step: add a timer to catch state changes triggered by signals */
  uv_timer_init(global_mainloop, &timer_id);
#if USE_TURN
  uv_timer_start(&timer_id, timer_cb, 300000, 0);
#else
  ...
  g_assert (result == 0);
  g_assert (global_lagent_state[0] == XICE_COMPONENT_STATE_READY);
  g_assert (global_lagent_state[1] == XICE_COMPONENT_STATE_READY);
  g_assert (global_ragent_state[0] == XICE_COMPONENT_STATE_READY);
  g_assert (global_ragent_state[1] == XICE_COMPONENT_STATE_READY);
  /* When using TURN, we get peer reflexive candidates for the host cands
     that we removed so we can get another new_selected_pair signal later
     depending on timing/racing, we could double (or not) the amount we expected
  */
  ...
  */
  ...
  priv_print_global_status ();
  g_assert (result == 0);
  g_assert (global_lagent_state[0] == XICE_COMPONENT_STATE_FAILED);
  g_assert (global_lagent_state[1] == XICE_COMPONENT_STATE_LAST);
  g_assert (global_ragent_state[0] == XICE_COMPONENT_STATE_FAILED);
  g_assert (global_ragent_state[1] == XICE_COMPONENT_STATE_LAST);
  ...
  g_assert (global_lagent_state[0] == XICE_COMPONENT_STATE_FAILED ||
	    global_lagent_state[1] == XICE_COMPONENT_STATE_FAILED);
  g_assert (global_lagent_state[0] == XICE_COMPONENT_STATE_FAILED ||
	    global_lagent_state[1] == XICE_COMPONENT_STATE_FAILED);
#endif
