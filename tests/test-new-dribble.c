...
send_buf:
  ...
  while (global_lagent_state < XICE_COMPONENT_STATE_CONNECTING) {
...
static void swap_candidates(XiceAgent *local, guint local_id, XiceAgent *remote, guint remote_id, gboolean signal_stun_reply)
{
  GSList *cands = NULL;
...
static void init_test(XiceAgent *lagent, XiceAgent *ragent, gboolean connect_new_candidate_signal)
{
