...
main (void)
{
  XiceContext *context;
  XiceAgent *agent;
  XiceAddress addr_local, addr_remote;
  ...
  xice_address_init (&addr_local);
  xice_address_init (&addr_remote);
  g_type_init ();
  ...
  g_assert (xice_address_set_from_string (&addr_local, "127.0.0.1"));
  ...
  xice_agent_add_local_address (agent, &addr_local);

  g_assert (agent->local_addresses != NULL);
  g_assert (g_slist_length (agent->local_addresses) == 1);
  g_assert (xice_address_equal (agent->local_addresses->data, &addr_local));
  ...
  candidates = xice_agent_get_local_candidates (agent, stream_id, 1);
  g_assert (g_slist_length (candidates) == 1);
  candidate = candidates->data;
  /* socket manager uses random port number */
  xice_address_set_port (&addr_local, 1);
  xice_address_set_port (&(candidate->addr), 1);
  g_assert (xice_address_equal (&(candidate->addr), &addr_local));
