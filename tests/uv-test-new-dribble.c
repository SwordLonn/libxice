...
uv_loop_t *global_mainloop = NULL;
uv_idle_t idler;

int* num_wait;
int num_target;

void wait_idle(uv_idle_t* handle) {
	if (*num_wait == num_target)
...
static int global_lagent_state = XICE_COMPONENT_STATE_LAST;
static int global_ragent_state = XICE_COMPONENT_STATE_LAST;
static GCancellable *global_cancellable;
static int exit_stun_thread = 0;
static int lagent_candidate_gathering_done = 0;
static int ragent_candidate_gathering_done = 0;
static guint global_ls_id, global_rs_id;
static int data_received = 0;
static int drop_stun_packets = 0;
static int got_stun_packet = 0;
static int send_stun = 0;
...
static void cb_component_state_changed(XiceAgent *agent, guint stream_id, guint component_id, guint state, gpointer data)
{
	...
	if (GPOINTER_TO_UINT(data) == 1 && state == XICE_COMPONENT_STATE_FAILED) {
	...
	if (GPOINTER_TO_UINT(data) == 1 && state == XICE_COMPONENT_STATE_READY) {
		/* note: test payload send and receive */
		ret = xice_agent_send(agent, stream_id, component_id,
...
static void swap_candidates(XiceAgent *local, guint local_id, XiceAgent *remote, guint remote_id, int signal_stun_reply)
{
	GSList *cands = NULL;

	g_debug("test-dribblemode:%s", G_STRFUNC);
	cands = xice_agent_get_local_candidates(local, local_id,
		XICE_COMPONENT_TYPE_RTP);
	g_assert(xice_agent_set_remote_candidates(remote, remote_id,
		XICE_COMPONENT_TYPE_RTP, cands));
...
static void cb_agent_new_candidate(XiceAgent *agent, guint stream_id, guint component_id, gchar *foundation, gpointer user_data)
{
	XiceAgent *other = g_object_get_data(G_OBJECT(agent), "other-agent");
	GSList *cands = xice_agent_get_local_candidates(agent, stream_id,
		component_id);
	GSList *i = NULL;
	...
	for (i = cands; i; i = i->next) {
		temp = (XiceCandidate*)i->data;
		if (g_strcmp0(temp->foundation, foundation) == 0) {
			g_debug("Adding new local candidate to other agent's connchecks");
			remote_cands = g_slist_prepend(remote_cands, xice_candidate_copy(temp));
			g_assert(xice_agent_set_remote_candidates(other, id,
				XICE_COMPONENT_TYPE_RTP,
				remote_cands));
		}
	}

	g_slist_free_full(remote_cands, (GDestroyNotify)xice_candidate_free);
	g_slist_free_full(cands, (GDestroyNotify)xice_candidate_free);

}

static void add_bad_candidate(XiceAgent *agent, guint stream_id, XiceCandidate *cand)
{
	XiceAddress bad_addr;
	GSList *cand_list = NULL;

	g_assert(xice_address_set_from_string(&bad_addr, "172.1.0.1"));

	cand = xice_candidate_new(XICE_CANDIDATE_TYPE_HOST);
	cand->stream_id = stream_id;
	cand->component_id = XICE_COMPONENT_TYPE_RTP;
	cand->addr = bad_addr;

	xice_agent_get_local_credentials(agent, stream_id,
		&cand->username, &cand->password);
	cand_list = g_slist_prepend(cand_list, cand);

	g_debug("Adding buggy candidate to the agent %p", agent);
	g_assert(xice_agent_set_remote_candidates(agent, stream_id,
		XICE_COMPONENT_TYPE_RTP,
		cand_list));

	g_slist_free_full(cand_list, (GDestroyNotify)xice_candidate_free);

}

static void init_test(XiceAgent *lagent, XiceAgent *ragent, int connect_new_candidate_signal)
{
	global_lagent_state = XICE_COMPONENT_STATE_DISCONNECTED;
	global_ragent_state = XICE_COMPONENT_STATE_DISCONNECTED;

	lagent_candidate_gathering_done = 0;
	ragent_candidate_gathering_done = 0;

	global_ls_id = xice_agent_add_stream(lagent, 1);
	global_rs_id = xice_agent_add_stream(ragent, 1);
	...
	xice_agent_attach_recv(lagent, global_ls_id, XICE_COMPONENT_TYPE_RTP,
		cb_xice_recv, LEFT_AGENT);
	xice_agent_attach_recv(ragent, global_rs_id, XICE_COMPONENT_TYPE_RTP,
		cb_xice_recv, RIGHT_AGENT);
}

static void cleanup(XiceAgent *lagent, XiceAgent *ragent)
{
	g_debug("Cleaning up");
	drop_stun_packets = 0;
	xice_agent_remove_stream(lagent, global_ls_id);
	xice_agent_remove_stream(ragent, global_rs_id);
}

static void wait_cond(int* num, int target) {
	num_target = target;
	num_wait = num;
	uv_idle_start(&idler, wait_idle);
	uv_run(global_mainloop, UV_RUN_DEFAULT);
}

static void standard_test(XiceAgent *lagent, XiceAgent *ragent)
{
	g_debug("test-dribblemode:%s", G_STRFUNC);

	got_stun_packet = 0;
	init_test(lagent, ragent, 0);

	xice_agent_gather_candidates(lagent, global_ls_id);
	wait_cond(&got_stun_packet, 1);

	g_assert(got_stun_packet);
	g_assert(global_lagent_state == XICE_COMPONENT_STATE_GATHERING &&
		!lagent_candidate_gathering_done);

	xice_agent_gather_candidates(ragent, global_rs_id);



	wait_cond(&ragent_candidate_gathering_done, 1);
	...
	wait_cond(&lagent_candidate_gathering_done, 1);
	g_cancellable_reset(global_cancellable);
...
static void bad_credentials_test(XiceAgent *lagent, XiceAgent *ragent)
{
	g_debug("test-dribblemode:%s", G_STRFUNC);

	init_test(lagent, ragent, FALSE);

	xice_agent_set_remote_credentials(lagent, global_ls_id,
		"wrong", "wrong");
	xice_agent_set_remote_credentials(ragent, global_rs_id,
		"wrong2", "wrong2");
	...
	swap_candidates(ragent, global_rs_id,
		lagent, global_ls_id,
		FALSE);
	wait_cond(&global_lagent_state, XICE_COMPONENT_STATE_FAILED);
	g_cancellable_reset(global_cancellable);
...
static void bad_candidate_test(XiceAgent *lagent, XiceAgent *ragent)
{
	...
	g_assert(global_lagent_state == XICE_COMPONENT_STATE_FAILED &&
		!data_received);
	set_credentials(lagent, global_ls_id, ragent, global_rs_id);
...
static void new_candidate_test(XiceAgent *lagent, XiceAgent *ragent)
{
	g_debug("test-dribblemode:%s", G_STRFUNC);
...
int main(void)
{
	XiceAgent *lagent = NULL, *ragent = NULL;
...
#if !GLIB_CHECK_VERSION(2,31,8)
	g_thread_init(NULL);
	stun_thread = g_thread_create(stun_thread_func, NULL, 1, NULL);
	...
	uv_idle_init(global_mainloop, &idler);
	xice_context_create("libuv", (gpointer)global_mainloop);
	XiceContext *tc = xice_context_create("libuv", (gpointer)global_mainloop);
	g_assert(tc != NULL);

	lagent = xice_agent_new(tc, XICE_COMPATIBILITY_RFC5245);
	ragent = xice_agent_new(tc, XICE_COMPATIBILITY_RFC5245);

	g_object_set(G_OBJECT(lagent), "controlling-mode", 1, NULL);
	g_object_set(G_OBJECT(ragent), "controlling-mode", 0, NULL);
	...
	g_assert(xice_address_set_from_string(&baseaddr, "127.0.0.1"));
